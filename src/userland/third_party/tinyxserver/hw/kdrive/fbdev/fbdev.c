 ...
 *
 * Copyright (c) 1999 Keith Packard
 *
...
fbdevInitialize (KdCardInfo *card, FbdevPriv *priv)
{
    int		    k;
    unsigned long   off;
    serial_printf("[Xfbdev] fbdevInitialize called\n");
    if ((priv->fd = open("/dev/fb0", O_RDWR)) < 0) {
	serial_printf("[Xfbdev] Error opening /dev/fb0, errno=%d\n", errno);
	return FALSE;
    }
    serial_printf("[Xfbdev] Opened /dev/fb0 successfully, fd=%d\n", priv->fd);
    if ((k=ioctl(priv->fd, FBIOGET_FSCREENINFO, &priv->fix)) < 0) {
	serial_printf("[Xfbdev] Error with /dev/fb ioctl FIOGET_FSCREENINFO, errno=%d\n", errno);
	close (priv->fd);
	return FALSE;
    }
    serial_printf("[Xfbdev] FSCREENINFO: smem_start=0x%lx, smem_len=%u, line_length=%u\n",
                  priv->fix.smem_start, priv->fix.smem_len, priv->fix.line_length);
    if ((k=ioctl(priv->fd, FBIOGET_VSCREENINFO, &priv->var)) < 0) {
	serial_printf("[Xfbdev] Error with /dev/fb ioctl FIOGET_VSCREENINFO, errno=%d\n", errno);
	close (priv->fd);
	return FALSE;
    }
    serial_printf("[Xfbdev] VSCREENINFO: xres=%u, yres=%u, bpp=%u\n",
                  priv->var.xres, priv->var.yres, priv->var.bits_per_pixel);

    serial_printf("[Xfbdev] Mapping framebuffer via mmap, size=%u...\n", priv->fix.smem_len);
    ...
    {
        serial_printf("[Xfbdev] ERROR: mmap framebuffer fails, errno=%d!\n", errno);
	close (priv->fd);
	return FALSE;
    }
    serial_printf("[Xfbdev] Framebuffer mapped successfully at %p\n", priv->fb_base);
    off = (unsigned long) priv->fix.smem_start % (unsigned long) getpagesize();
    priv->fb = priv->fb_base + off;
    serial_printf("[Xfbdev] Framebuffer pointer offset: off=%lu, final_ptr=%p\n", off, priv->fb);
    return TRUE;
}
...
Bool fbdevRandRSetConfig (ScreenPtr		pScreen,
		     Rotation		rotateKind,
		     RRScreenSizePtr	pSize,
		     RRVisualGroupPtr	pVisualGroup)
{
